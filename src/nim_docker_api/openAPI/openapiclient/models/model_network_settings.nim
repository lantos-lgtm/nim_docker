...
type NetworkSettings* = object
  ...
  hairpinMode*: bool ## Indicates if hairpin NAT should be enabled on the virtual interface. 
  linkLocalIPv6Address*: string ## IPv6 unicast address using the link-local prefix.
  linkLocalIPv6PrefixLen*: int ## Prefix length of the IPv6 unicast address.
  ports*: Table[string, seq[PortBinding]] ## PortMap describes the mapping of container ports to host ports, using the container's port-number and protocol as key in the format `<port>/<protocol>`, for example, `80/udp`.  If a container's port is mapped for multiple protocols, separate entries are added to the mapping table. 
  sandboxKey*: string ## SandboxKey identifies the sandbox
  secondaryIPAddresses*: seq[Address] ## 
  secondaryIPv6Addresses*: seq[Address] ## 
  ...
  networks*: Table[string, EndpointSettings] ## Information about all networks that the container is connected to. 
