  ...
  if client.currentURL.hostname != url.hostname or
    ...
    when client is HttpClient:
      if connectionUrl.scheme == "unix":
        ...
        client.socket.connectUnix(connectionUrl.hostname)
      else:
...
proc responseContent(resp: Response | AsyncResponse): Future[
    string] {.multisync.} =
...
proc delete*(client: HttpClient | AsyncHttpClient;
             url: Uri | string): Future[Response |
...
proc downloadFileEx(client: AsyncHttpClient;
                    url: Uri | string; filename: string): Future[
                        void] {.async.} =
  ## Downloads `url` and saves it to `filename`.
  client.getBody = false
