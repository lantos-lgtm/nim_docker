...
export interface ServiceSpecRollbackConfig {
  ...
  parallelism:Number // Maximum number of tasks to be rolled back in one iteration (0 means unlimited parallelism). 
  delay:Number // Amount of time between rollback iterations, in nanoseconds. 
  failureAction: FailureAction // Action to take if an rolled back task fails to run, or stops running during the rollback. 
  monitor:Number // Amount of time to monitor each rolled back task for failures, in nanoseconds. 
  maxFailureRatio:Number // The fraction of tasks that may fail during a rollback before the failure action is invoked, specified as aNumbering point number between 0 and 1. 
