import {SystemInfoDefaultAddressPoolsInner} from "./model_system_info_default_address_pools_inner.nim";
import {SwarmInfo} from "./model_swarm_info.nim";
import {Runtime} from "./model_runtime.nim";
import {RegistryServiceConfig} from "./model_registry_service_config.nim";
import {PluginsInfo} from "./model_plugins_info.nim";
import {GenericResourcesInner} from "./model_generic_resources_inner.nim";
import {Commit} from "./model_commit.nim";
//
// Docker Engine API
// 
...
export interface SystemInfo {
  ...
  containers:Number // Total number of containers on the host.
  containersRunning:Number // Number of containers with status \"running\". 
  containersPaused:Number // Number of containers with status \"paused\". 
  containersStopped:Number // Number of containers with status \"stopped\". 
  images:Number // Total number of images on the host.  Both _tagged_ and _untagged_ (dangling) images are counted. 
  ...
  nFd:Number // The total number of file Descriptors in use by the daemon process.  This information is only returned if debug-mode is enabled. 
  nGoroutines:Number // The  number of goroutines that currently exist.  This information is only returned if debug-mode is enabled. 
  ...
  nEventsListener:Number // Number of event listeners subscribed.
  ...
  oSType: string // Generic  of the operating system of the host, as returned by the Go runtime (GOOS).  Currently returned values are \"linux\" and \"windows\". A full list of possible values can be found in the [Go documentation](https://golang.org/doc/install/source//environment). 
  ...
  NCPU:Number // The number of logical CPUs usable by the daemon.  The number of available CPUs is checked by querying the operating system when the daemon starts. Changes to operating system CPU allocation after the daemon is started are not reflected. 
  memTotal:Number // Total amount of physical memory available on the host, in bytes. 
  ...
  genericResources: [GenericResources_inner] // User-defined resources can be either Integer resources (e.g, SSD=3) or String resources (e.g, GPU=UUID1). 
  ...
  runtimes: {[key:string]: Runtime} // List of [OCI compliant](https://github.com/opencontainers/runtime-spec) runtimes configured on the daemon. Keys hold the \"name\" used to reference the runtime.  The Docker daemon relies on an OCI compliant runtime (invoked via the containerd daemon) as itsNumbererface to the Linux kernel namespaces, cgroups, and SELinux.  The default runtime is runc, and automatically configured. Additional runtimes can be configured by the user and will be listed here. 
  ...
  runcCommit: Commit
  initCommit: Commit
  securityOptions: [string] // List of security features that are enabled on the daemon, such as apparmor, seccomp, SELinux, user-namespaces (userns), and rootless.  Additional configuration options for each security feature may be present, and are included as a comma-separated list of key/value pairs. 
  ...
  defaultAddressPools: [SystemInfo_DefaultAddressPools_inner] // List of custom default address pools for local networks, which can be specified in the daemon.json file or dockerd option.  Example: a Base \"10.10.0.0/16\" with Size 24 will define the set of 256 10.10.[0-255].0/24 address pools. 
