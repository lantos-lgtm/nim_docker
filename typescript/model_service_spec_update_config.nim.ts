...
export interface ServiceSpecUpdateConfig {
  ...
  parallelism:Number // Maximum number of tasks to be updated in one iteration (0 means unlimited parallelism). 
  delay:Number // Amount of time between updates, in nanoseconds.
  failureAction: FailureAction // Action to take if an updated task fails to run, or stops running during the update. 
  monitor:Number // Amount of time to monitor each updated task for failures, in nanoseconds. 
  maxFailureRatio:Number // The fraction of tasks that may fail during an update before the failure action is invoked, specified as aNumbering point number between 0 and 1. 
