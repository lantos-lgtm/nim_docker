import {ImageInspectRootFS} from "./model_image_inspect_root_fs.nim";
import {ImageInspectMetadata} from "./model_image_inspect_metadata.nim";
import {GraphDriverData} from "./model_graph_driver_data.nim";
import {ContainerConfig} from "./model_container_config.nim";
//
// Docker Engine API
// 
...
export interface ImageInspect {
  ...
  repoTags: [string] // List of image names/tags in the local image cache that reference this image.  Multiple image tags can refer to the same imagem and this list may be empty if no tags reference the image, in which case the image is \"untagged\", in which case it can still be referenced by its ID. 
  ...
  size:Number // Total size of the image including all layers it is composed of. 
  virtualSize:Number // Total size of the image including all layers it is composed of.  In versions of Docker before v1.10, this field was calculated from the image itself and all of its parent images. Docker v1.10 and up store images self-contained, and no longer use a parent-chain, making this field an equivalent of the Size field.  This field is kept for backward compatibility, but may be removed in a future version of the API. 
  graphDriver: GraphDriverData
  rootFS: ImageInspect_RootFS
  metadata: ImageInspect_Metadata
}
